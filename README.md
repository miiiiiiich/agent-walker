...
- **Antigravity tokens are read from an undocumented protobuf** (decoded by
  field number and self-verified per row). They count toward the totals, but its
  **cost shows $0** — its gemini model ids aren't in the pricing table yet; see
